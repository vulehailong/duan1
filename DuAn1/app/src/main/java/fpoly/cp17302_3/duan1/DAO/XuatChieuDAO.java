...
import fpoly.cp17302_3.duan1.Model.XuatChieu;
...
public class XuatChieuDAO {
    ...
    public ArrayList<XuatChieu> getDSXuatChieu(){
        ...
        if (cursor.getCount() != 0){
            cursor.moveToFirst();
            do {
                list.add(new XuatChieu(cursor.getString(0), cursor.getString(1), cursor.getString(2)));
    ...
    public boolean capNhatXuatChieu(XuatChieu xuatChieu){
        ...
        long check = sqLiteDatabase.update("XUATCHIEU", contentValues, "maxuatchieu = ?", new String[]{xuatChieu.getMaxuatchieu()});
        if (check == -1){
            return false;
        }else {
            return true;
